...
public class RNAliPlayerManager extends ViewGroupManager<AliSurfaceView> {
    ...
    @Override
    protected AliSurfaceView createViewInstance(ThemedReactContext context) {
    ...
    public void receiveCommand(AliSurfaceView view, String command, @Nullable ReadableArray args) {
        ...
        switch (command) {
            ...
            case "playVidSts":
                ...
                vidSts.setVid(args.getString(0));
                vidSts.setAccessKeyId(args.getString(1));
                vidSts.setAccessKeySecret(args.getString(2));
                vidSts.setSecurityToken(args.getString(3));
                vidSts.setRegion(args.getString(4));
                view.aliyunVodPlayer.setDataSource(vidSts);
                view.aliyunVodPlayer.prepare();
                break;
            case "playLocalPath":
                //播放本地路径
                String jsonSrc=args.getString(0);
                File file = new File(jsonSrc);
                if (file.exists()) {
                    UrlSource source = new UrlSource();
                    source.setUri(jsonSrc);
    ...
    public void setSrc(AliSurfaceView view, String jsonSrc) {
        if (!TextUtils.isEmpty(jsonSrc)){
            //CacheConfig 需要在prepare前设置
            CacheConfig cacheConfig = new CacheConfig();
            //开启缓存功能
            cacheConfig.mEnable = true;
            //能够缓存的单个文件最大时长。超过此长度则不缓存
            cacheConfig.mMaxDurationS = 99999999;
            //缓存目录的位置
            String path = view.mContext.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS).getPath();
            cacheConfig.mDir = path;
            //缓存目录的最大大小。超过此大小，将会删除最旧的缓存文件
            cacheConfig.mMaxSizeMB = 99999999;
            //设置缓存配置给到播放器
            view.aliyunVodPlayer.setCacheConfig(cacheConfig);
            Log.i(TAG, "setSrc: " + jsonSrc);
            //本地路径
//        String filename = "f25850866197484ba7d66dc85115fd56_0.mp4";
            String fpath = view.mContext.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS).getPath();
            if (jsonSrc.contains(fpath)) {
                File file = new File(jsonSrc);
                if (file.exists()) {
                    UrlSource source = new UrlSource();
                    source.setUri(jsonSrc);
            ...
            } else if (jsonSrc.contains("http")) {
                try {
                    UrlSource source = new UrlSource();
                    String sourceStr = jsonSrc.replaceAll(" ", "%20");
                ...
                Log.i(TAG, "setVid: " + jsonSrc);
                try {
                    Gson gson=new Gson();
                    VidSts vidSts=gson.fromJson(jsonSrc,VidSts.class);
                    // VidAuth vidAuth=gson.fromJson(jsonSrc,VidAuth.class);
                    view.aliyunVodPlayer.setDataSource(vidSts);
                    view.aliyunVodPlayer.prepare();
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    private void initListener(final AliSurfaceView view) {
        ...
        view.aliyunVodPlayer.setOnPreparedListener(new IPlayer.OnPreparedListener() {
            ...
            public void onPrepared() {
                ...
                prepareEvent.putInt("height", height);
